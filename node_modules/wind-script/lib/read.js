...
function getFileList(dirPath = ".", last = "ws") {
    ...
    getDir.forEach((value) => {
        ...
        if (fs.statSync(filePath).isDirectory()) {
            returnDir.line = returnDir.line.concat(getFileList(filePath).line);
            returnDir.name = returnDir.name.concat(getFileList(filePath).name);
            returnDir.path = returnDir.path.concat(getFileList(filePath).path);
        }
    })
    return returnDir;
}
