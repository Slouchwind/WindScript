...
function objectToTable(object, output) {
    var returnTable = [`i${" ".repeat(String(object[output[0]].length).length - 1)}`];
    for (let i = 0; i < object[output[0]].length; i++) {
        returnTable[returnTable.length] = `${i}${" ".repeat(String(object[output[0]].length).length - String(i).length)}`;
    }
    for (let index = 0; index < output.length; index++) {
        var getTable = object[output[index]];
